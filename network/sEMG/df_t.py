...
class HNFBlock(nn.Module):
    ...
    def forward(self, x):
        residual = x
        filts = [layer(x) for layer in self.filters]
...
class ConditionalModel_t(nn.Module):
    def __init__(self, feats=64):
        super(ConditionalModel_t, self).__init__()
    ...
    def forward(self, x):
        xs = []
        for layer in self.stream_x:
            x = layer(x)
            xs.append(x)
        x = self.conv_out(xs[-1])
        x = torch.mean(x, dim=2)  # 添加全局平均池化
        x = torch.sigmoid(x)      # 添加 sigmoid 激活函数
        return x.squeeze(1)

if __name__ == '__main__':
    model = ConditionalModel_t(feats=64)
    input_size = (1, 10000)
    batch_size = 256
